...
- **Drafts live on the local-only `draft` branch, never on `main`.** Drafts carry `draft = true`; write and commit them on `draft` (the only place rendering can be validated: `zola-plus serve --drafts`). The branch is never pushed — the repo source is public — and is guarded twice: `branch.draft.pushRemote` points at a nonexistent remote, and the pre-push hook (`scripts/pre-push-no-drafts.sh` → install as `.git/hooks/pre-push`) blocks any pushed tree whose tip contains a draft. Rules: non-draft work (templates, config, CSS, typo fixes) happens on `main`; sync with `git switch draft && git merge main` regularly. **Publishing**: on `main`, `git restore --source=draft -- content/<slug>.md` (plus any template/JS the article needs), remove `draft = true`, set final `date`, commit, push; then on `draft`, merge main and delete/normalize the leftover draft copy. Research/positioning for launch articles lives in the private `../outreach` repo.
